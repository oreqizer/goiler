...
var (
	accountDBTypes = map[string]string{`ID`: `uuid`, `AuthID`: `text`, `Name`: `text`, `Surname`: `text`, `Email`: `text`, `IsAdmin`: `boolean`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_              = bytes.MinRead
)
